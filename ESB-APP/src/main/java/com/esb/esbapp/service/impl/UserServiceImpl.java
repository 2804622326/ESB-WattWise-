...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Map<String, Number> getUserTotalPointsAndMonthlyEnergy(Long userId) {
        User user = getUserById(userId);
        Map<String, Number> result = new HashMap<>();
        result.put("totalPoints", user.getTotalPoints());
        result.put("monthlyEnergy", user.getMonthlyEnergy());
        return result;
    }

    @Override
    public Map<String, Number> getCommunityTotalPointsAndMonthlyEnergy() {
        List<User> users = userRepository.findAll();
        int totalPointsSum = users.stream().mapToInt(User::getTotalPoints).sum();
        double monthlyEnergySum = users.stream().mapToDouble(User::getMonthlyEnergy).sum();

        Map<String, Number> result = new HashMap<>();
        result.put("totalPoints", totalPointsSum);
        result.put("monthlyEnergy", monthlyEnergySum);
        return result;
    }
}